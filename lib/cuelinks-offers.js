...
export async function fetchCuelinksOffers(limit = 24) {
  ...
  try {
    ...
    const campaignIds = [
      ...new Set(
        selected.map((offer) => offer.campaignId).filter(Number.isFinite)
      ),
    ];
      ...
      await Promise.all(
        campaignIds.map(async (campaignId) => {
          try {
            ...
            if (!campaignResponse.ok) return [campaignId, ""];

            const campaignPayload = await campaignResponse.json();
            return [campaignId, safeHttpsUrl(campaignPayload?.data?.image)];
          } catch {
            return [campaignId, ""];
          }
        })
    ...
    return selected.map((offer) => ({
      ...offer,
      image: campaignImages.get(offer.campaignId) || "",
    }));
  } catch {
    return [];
  }
